...
(function($){
	...
	function Page(container,fn,args,style){
		...
		var args=this.args=$.extend({
			...
			pageSwap:false,//是否同意调剂,默认是。调剂会最大数量显示页码。例如当前页码之前无页码可以显示。则会增加之后的页码。
	...
	Page.prototype.fillHtml=function(){
		...
		//跳转页码
		if(args.showTurn){
			obj.append('<span class="countYe">到第<input type="text" class="inp-search" maxlength='+args.pageCount.toString().length+'>页<a href="javascript:;" class="turndown">确定</a></span>');
		}
	...
	Page.prototype.bindEvent=function(){
		...
		var _this=this;
			...
			obj.on("click","a.tcdNumber",function(){
				...
				console.log(_this.args.current);
				_this.fillHtml();
			});
			//上一页
			...
			obj.on("click","a.turndown",function(){
				var _i=".pl"+$(this).parent().siblings(".current").text();
				$(_i).hide();
	...
	function abc(o){
		var _i=".pl"+o.siblings(".current").text();
			$(_i).hide();
	}
...
$(function(){
	$('.page3').createPage(function(n){
			console.log(n);
			var _class=".pl"+n;
			var ele=document.querySelector(_class);
			if(ele){
				$(_class).show();
			}else{
				$(".pl1").show();
